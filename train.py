...
def get_data(config, batch_size=256, workers=6):

    data_paths = config['file_paths']
    entry_ranges = config['entry_ranges']

    ### Load datasets
    datasets = {}
    for split, data_path in data_paths.items():
        if data_path.endswith('.h5') or data_path.endswith('.hdf5'):
            datasets[split] = CaloChallengeDataset(data_path, config, 
                                                   entry_start=entry_ranges[split][0], 
                                                   entry_stop=entry_ranges[split][1])
        elif data_path.endswith('.root'):
            datasets[split] = CaloDataset(data_path, downsample_factors=[4,4,4], layer=None)
        else:
    ...
    dataloaders = {k: DataLoader(v, batch_size=batch_size, 
                                    shuffle=(k=='train'), 
                                    num_workers=workers)
                                for k, v in datasets.items() if len(v) > 0
                    }
...
def train(ofm_model, dls, args):
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=args.epochs//10, gamma=0.7)
...
def generate(ofm_model, dl_test, config, Nmax=10000):

    n_eval = 24
    method = 'euler'

    Nvoxels = config['dims'][0] * config['dims'][1] * config['dims'][2]
...
if __name__ == "__main__":
    ...
    ds, dls = get_data(config, args.batch_size)
    ...
    if args.mode == 'train':
        ...
        generate(ofm_model, dls['test'], config, Nmax=len(ds['test']))